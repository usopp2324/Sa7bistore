...
const sanitizeOrderId = (value) => {
    ...
    // Keep letters, numbers and hyphens only, then uppercase
...
module.exports = {
    ...
    async execute(message, args, context) {
        ...
        try {
            console.debug('pay: POST', `${config.djangoApiBaseUrl}/api/discord/order/paid/`, payload);
            await postToDjango('/api/discord/order/paid/', payload, config.djangoApiBaseUrl, config.apiSecret);
        } catch (err) {
            console.error('discord_order_paid request failed:', err);
            throw err;
        }
